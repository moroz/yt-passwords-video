...
In reality, there are many situations when you need to implement complex logic that doesn't seem to fit in either the model or the controller.
...
<!-- re-record this line and call it a panic -->
...
Then, define a struct called `UserService` with a private field called `store`, which will hold a `UserStore` struct.
Define a function called `NewUserService`, which takes a pointer to a database connection and returns a `UserService` struct.
This will be our constructor that we will use to initialize `UserService` structs.
Then, define a method on the `UserService` type that takes an email and a password and returns a pointer to a `User` struct and an error.
...
Finally, if this check has passed, it means that the user exists and that the password is correct, so we can return the user and no error.

## Testing `UserService`

Now, let us write some tests for the `UserService` struct.
We're going to have to create a file named `service_test.go` in the `service` directory.
The file will be almost exactly the same as the `store_test.go` file in the `store` directory, so I'm not going to bother writing it out by hand.

```
sed -e 's/Store/Service/g;s/store/service/g' store/store_test.go | tee service/service_test.go
```

Instead, I am going to use a command line tool called `sed`. This command will read the file `store_test.go`, replace every occurrence of "Store" with "Service," and print it out to the standard output. Now, I pipe it to a command called `tee`, which will also write this text to the file I passed as its argument.
...
Define a method called `TestAuthenticateUserByEmailPassword` on the `ServiceTestSuite` struct. This will be a test example that the test suite will execute.
...
Then, if we pass the correct email and an invalid password, we should get a `nil` user and `ErrInvalidPassword`.
If we pass the correct password and a non-existent email address, we should get a `nil` user and an `ErrNoRows` error.
...
In this video, we have implemented a few functions related to database operations and password hashing.
In the next video, we can take this knowledge further and implement user registration and login views.
If you enjoyed this video, please like this video, it really helps with the YouTube algorithm, and if you are feeling very generous, please consider subscribing to my channel, so I can become rich and famous.
