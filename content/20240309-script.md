...
In reality, there are many situations when you need to implement complex logic that doesn't seem to fit in either the model or the controller. Therefore, some frameworks have come up with another layer, which sits in between the controller and the model. In Phoenix, which is a Web framework for Elixir, these are called "contexts." In Rails, you can use a pattern called "services." In this video, we are not working with a framework, so we can structure the code however we want.
We will be writing a "store," containing the logic responsible for simple database interactions. On top of the store, we will implement a "service," which will handle the more complex logic, such as the password authentication workflow we are going to implement today.
In this setup, a controller handles incoming HTTP requests, fetches and persists data using a model, and returns them to the client using a view module.
In this video we are only going to be dealing with the model layer, which is responsible for database interactions and data validations.
...
Now, let's implement a "user service". In this setup, a service acts as a link between a store and a request handler. If 
